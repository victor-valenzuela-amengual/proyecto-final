...
const registrarUsuario = async (data) => {
  try {
    let { nombre, direccion, fono, password, email } = data;
    const passwordEncriptada = bcrypt.hashSync(password, 10);
    password = passwordEncriptada;
    const values = [nombre, direccion, fono, passwordEncriptada, email];
    console.log(values);
    const consulta =
      "insert into usuario (nombre,direccion,fono,password,correo) " +
      " values ($1,$2,$3,$4,$5)";
